...
/* =========================================================
   NEW IMAGE GENERATION LOGIC (Fully Upgraded to Match R-Gen API)
========================================================= */
async function generateImage(prompt) {
    const encodedPrompt = encodeURIComponent(prompt);
    const uniqueId = Date.now();
    
    // Default style and camera for chat interface functionality
    const style = 'none'; 
    const camera = 'none';
    ...
    try {
        ...
        // Fetch exactly like the HTML script
        ...
        // Premium Dark-Themed UI integrated into the chat bubble 
        // Note: Starts with `<div` and contains `<img` so it renders correctly natively.
        return `
        ...
        // Fallback UI properly formatted to bypass chat markdown parser correctly
        return `
        <div style="border: 1px solid #ef4444; color: #ef4444; background: rgba(239, 68, 68, 0.1); padding: 12px; border-radius: 8px; font-family: sans-serif;">
            ❌ Error: ${error.message}
            <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" style="display:none;" alt="error">
        </div>`;
    }
}

// Exactly matches HTML logic (forces blob download + smart fallback on CORS errors)
