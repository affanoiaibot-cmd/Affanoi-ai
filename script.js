...
function handleHashRouting() {
    ...
    // 1. Handle dynamic blog posts (#blog-heuhebheu828)
...
/* =========================================================
   NEW IMAGE GENERATION LOGIC (Dual-Engine Auto Fallback)
========================================================= */
async function generateImage(prompt) {
    ...
    try {
        // ENGINE 1: Try your original R-Gen API first
        const apiUrl = `https://r-gengpt-api.vercel.app/api/image?prompt=${encodedPrompt}&style=none&camera=none`;
        
        // Timeout wrapper so it doesn't freeze the chat if API is offline
        ...
        console.warn("Primary R-Gen API unavailable, seamlessly switching to backup engine...");
    }

    // ENGINE 2: Bulletproof Fallback
    // If the R-Gen API fails or times out, it will automatically use Pollinations AI
    if (!imageUrl) {
        imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?nologo=true&enhance=true`;
    }

    // Returns the exact Premium Dark-Themed UI you requested
    return `
    <div class="image-wrapper" style="background: rgba(30, 41, 59, 0.7); backdrop-filter: blur(10px); padding: 15px; border-radius: 16px; border: 1px solid rgba(255, 255, 255, 0.1); width: 100%; max-width: 340px; display: flex; flex-direction: column; gap: 15px; box-shadow: 0 10px 30px -10px rgba(0,0,0,0.5); font-family: sans-serif;">
        
        <!-- Loading Interface -->
        <div id="img-loader-${uniqueId}" style="height: 250px; width: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; border-radius: 12px; background: rgba(2, 6, 23, 0.5); border: 2px dashed rgba(255,255,255,0.1);">
            <div style="width: 40px; height: 40px; border: 4px solid rgba(139, 92, 246, 0.1); border-left-color: #8b5cf6; border-radius: 50%; animation: rgen-spin 1s linear infinite;"></div>
            <span style="font-size: 0.85em; margin-top: 15px; color: #c084fc; font-weight: 500;">AI Engine Processing...</span>
            <style>@keyframes rgen-spin { 100% { transform: rotate(360deg); } }</style>
        </div>
        
        <!-- Displaying Image -->
        <img src="${imageUrl}" class="message-image" alt="Generated Art" 
            onload="document.getElementById('img-loader-${uniqueId}').style.display='none'; this.style.display='block'; setTimeout(() => document.getElementById('chatMessages').scrollTo({top: document.getElementById('chatMessages').scrollHeight, behavior: 'smooth'}), 100);" 
            onerror="this.style.display='none'; document.getElementById('img-loader-${uniqueId}').innerHTML='<span style=\\'color:#ef4444; font-weight:500;\\'>❌ Failed to load image</span>';" 
            onclick="openImageViewer(this.src)" 
            style="width: 100%; height: auto; border-radius: 12px; cursor: pointer; display: none; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
            
        <!-- Download Action Button -->
        <button onclick="downloadGeneratedImage('${imageUrl}', 'R-GenGPT-${uniqueId}.jpg', this)" style="width: 100%; padding: 14px; background: linear-gradient(135deg, #8b5cf6, #3b82f6); color: white; border: none; border-radius: 10px; cursor: pointer; font-size: 0.95em; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 15px rgba(139, 92, 246, 0.3); transition: transform 0.2s;">
...
// Exactly matches HTML logic (Fixed CORS errors to ensure downloads ALWAYS work)
async function downloadGeneratedImage(url, filename, btnElement) {
    const originalText = btnElement.innerHTML;
    
    // Set downloading animation directly on button
    btnElement.innerHTML = `
        <div style="width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-left-color: #fff; border-radius: 50%; animation: rgen-spin 1s linear infinite;"></div>
        Downloading...
    `;
    ...
    try {
        ...
        try {
            ...
            // Attempt 2: CORS Proxy Bypass (Fixes "Download Failed" errors)
        ...
        btnElement.innerHTML = '✅ Downloaded Successfully!';
        if(typeof showToast === 'function') showToast('✅ Downloaded Successfully!');
        
    } catch (error) {
        console.error('Download error:', error);
        btnElement.innerHTML = '❌ Download Failed';
        if(typeof showToast === 'function') showToast('❌ Failed to download. Try clicking the image to open it.');
        
        // Native fallback alert explaining to the user
        alert("Failed to download directly due to browser restrictions. Please click the image to open it, then Long-press or Right-click and select 'Save Image As'.");
    } finally {
        setTimeout(() => {
...
function openBlogModal(specificBlogId = null, fromHash = false) {
    ...
    db.ref('blog').once('value').then((snapshot) => {
        ...
        // 100% Fail-safe mapping - no ES6 Spread Operator used so it won't crash older devices
        const posts = [];
        const keys = Object.keys(data);
        for (let i = 0; i < keys.length; i++) {
            let key = keys[i];
            let postItem = data[key];
            if (postItem && typeof postItem === 'object') {
                postItem.uniqueId = key;
                posts.push(postItem);
            }
        }
        posts.reverse();
        ...
        posts.forEach((post) => {
            ...
            const shareLink = `${baseUrl}#blog-${postId}`; 
            ...
            // Fixes potential crashes caused by titles with unescaped quote marks
            const safeTitle = (post.title || 'EduMate Update').replace(/'/g, "\\'").replace(/"/g, "&quot;");
            
            let shareBtnHtml = `<button class="blog-share-btn" onclick="shareBlogPost('${safeTitle}', '${shareLink}')"><i class="fas fa-share-alt"></i> Share Update</button>`;
        ...
        // Directly outputs the actual error reason to help debug if it fails again
        container.innerHTML = `<div class="loading-text" style="color: #ef4444; padding: 15px;">Error: ${error.message}</div>`; 
    });
}
